...
public class TargetDrawable {
	...
	/**
	 * This is changed from the framework version to pass in the number of
	 * drawables in the container. The framework version relies on private api's
	 * to get the count from StateListDrawable.
	 * 
	 * @param res
	 * @param resId
	 * @param count
	 *            The number of drawables in the resource.
	 */
	...
	public void setDrawable(Resources res, int resId) {
		// Note we explicitly don't set mResourceId to resId since we allow the
		// drawable to be
		// swapped at runtime and want to re-use the existing resource id for
		// identification.
	...
	private void resizeDrawables() {
		if (mDrawable instanceof StateListDrawable) {
			...
			for (int i = 0; i < mNumDrawables; i++) {
				...
				maxWidth = Math
						.max(maxWidth, childDrawable.getIntrinsicWidth());
				maxHeight = Math.max(maxHeight,
						childDrawable.getIntrinsicHeight());
			}
