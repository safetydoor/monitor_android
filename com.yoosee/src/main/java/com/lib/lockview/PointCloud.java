...
public class PointCloud {
	...
	public int getAlphaForPoint(Point point) {
		...
		float glowDistance = hypot(glowManager.x - point.x, glowManager.y
				- point.y);
		...
		if (glowDistance < glowManager.radius) {
			float cosf = FloatMath.cos(PI * 0.25f * glowDistance
					/ glowManager.radius);
			glowAlpha = glowManager.alpha
					* max(0.0f, (float) Math.pow(cosf, 10.0f));
		}
		...
		if (distanceToWaveRing < waveManager.width * 0.5f
				&& distanceToWaveRing < 0.0f) {
			float cosf = FloatMath.cos(PI * 0.25f * distanceToWaveRing
					/ waveManager.width);
			waveAlpha = waveManager.alpha
					* max(0.0f, (float) Math.pow(cosf, 20.0f));
		}
